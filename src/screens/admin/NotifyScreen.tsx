    ...
    <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
      ...
      >
        ...
        <View style={empStyles.tokenWrap}>
          {hasToken ? <PulseDot color={C.green} size={8} /> : <View style={empStyles.tokenOff} />}
        </View>
      </TouchableOpacity>
    </Animated.View>
  );
...
export default function NotifyScreen() {
  ...
  const fetchEmployees = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(data) ? data.filter(e => e.role === 'employee') : [];
      setEmployees(list);
    } catch (e: any) {
      console.log('[NotifyScreen] fetchEmployees error:', e);
      haptic.error();
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await apiPost<NotifyResult>('/admin/notify', {
